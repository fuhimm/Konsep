...
const HallScene = () => {
  return (
    <group>
      ...
      {artworksData.map((art) => (
        // @ts-ignore
        <Artwork key={art.id} position={art.position} rotation={art.rotation} url={art.url} title={art.title} />
      ))}

      {/* Intro Text */}
      <group position={[0, 4, -28]}>
        <Text fontSize={1} color="black" anchorY="bottom">
          Fuhim
        </Text>
        <Text position={[0, -0.8, 0]} fontSize={0.25} color="#666" maxWidth={6} textAlign="center">
          Walk back to exit.
        </Text>
      </group>
      ...
      {galleryWallData.map((art, index) => (
        <group key={art.id}>
          <group position={[art.x, 3, -1.4]}>
            {/* Canvas */}
            <Artwork url={art.url} title={art.title} position={[0, 0, 0]} rotation={[0, 0, 0]} scale={[1.2, 1.2, 1]} />
